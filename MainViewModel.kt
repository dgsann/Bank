...
// Структура товара магазина
data class ShopItem(val id: String, val name: String, val price: Double, val icon: String, val description: String)
...
class MainViewModel(private val storage: AvatarStorage) : ViewModel() {
    ...
    val shopItems = listOf(
        ShopItem("sneakers", "Кроссовки", 15000.0, "👟", "Спорт тратит меньше сил (-10 эн.)"),
        ShopItem("coffee", "Кофемашина", 20000.0, "☕", "Утро добрее (+20 настр.)"),
        ShopItem("cat", "Рыжий Кот", 30000.0, "🐱", "Защита от грусти (>30 настр.)"),
        ShopItem("laptop", "MacBook Pro", 100000.0, "💻", "Учеба эффективнее (x2 Ума!)")
    )
    ...
    fun buyItem(item: ShopItem) {
        ...
        if (currentState.inventory.contains(item.id)) { _errorEvent.value = "Уже куплено!"; return }
        if (currentState.balance < item.price) { _errorEvent.value = "Не хватает денег!"; return }

        _state.update {
            val newState = it.copy(balance = it.balance - item.price, inventory = it.inventory + item.id)
            storage.saveState(newState)
            newState
        }
        _history.update { listOf("🛍️ Купил ${item.name} (-${item.price.toInt()}₽)") + it }
    }

    fun hasItem(id: String): Boolean = _state.value.inventory.contains(id)

    // --- ФИНАНСЫ (ВКЛАД И КРЕДИТ) ---
    fun investMoney(amount: Double) {
        _state.update {
            if (it.balance < amount) { _errorEvent.value = "Недостаточно наличных!"; return@update it }
            val ns = it.copy(balance = it.balance - amount, depositBalance = it.depositBalance + amount)
            storage.saveState(ns); ns
        }
    }

    fun withdrawMoney(amount: Double) {
        _state.update {
            if (it.depositBalance < amount) { _errorEvent.value = "Мало средств на вкладе!"; return@update it }
            val ns = it.copy(balance = it.balance + amount, depositBalance = it.depositBalance - amount)
            storage.saveState(ns); ns
        }
    }
    ...
    fun takeLoan(amount: Double) {
        if (_state.value.creditScore < 200) { _errorEvent.value = "Банк отказал (низкий рейтинг)!"; return }
        val maxLoan = getCurrentJob().salary * 10
        if (_state.value.loanBalance + amount > maxLoan) { _errorEvent.value = "Лимит кредита превышен!"; return }

        _state.update {
            val ns = it.copy(balance = it.balance + amount, loanBalance = it.loanBalance + amount)
            storage.saveState(ns); ns
        }
    ...
    fun repayLoan(amount: Double) {
        ...
        _state.update {
            if (it.balance < payment) { _errorEvent.value = "Недостаточно средств!"; return@update it }
            val ns = it.copy(balance = it.balance - payment, loanBalance = it.loanBalance - payment, creditScore = (it.creditScore + 2).coerceAtMost(850))
            storage.saveState(ns); ns
        }
    ...
    fun getCurrentInterestRate(): Double {
        ...
        return 1.0 + ((score - 300).coerceAtLeast(0) / 100.0) * 0.5
    }

    // --- ТРАНЗАКЦИИ (С БОНУСАМИ) ---
    fun onTransaction(category: TransactionCategory, amount: Double, description: String) {
        _state.update { currentState ->
            if (currentState.balance < amount) { _errorEvent.value = "Недостаточно средств!"; return@update currentState }
            if (category == TransactionCategory.SPORT && currentState.energy < 20) { _errorEvent.value = "Слишком устал!"; return@update currentState }

            // Бонусы инвентаря
            val hasSneakers = currentState.inventory.contains("sneakers")
            val hasLaptop = currentState.inventory.contains("laptop")

            var newState = EvolutionEngine.calculateNewState(currentState, category, amount)

            // Бонус Макбука: Ум растет в 2 раза быстрее
            if (category == TransactionCategory.EDUCATION && hasLaptop) {
                val diff = newState.intellect - currentState.intellect
                newState = newState.copy(intellect = newState.intellect + diff)
            }
            ...
            newState = when(category) {
                TransactionCategory.SPORT -> {
                    val energyCost = if (hasSneakers) 10 else 20 // Бонус Кроссовок
                    newState.copy(spentOnSport = newState.spentOnSport + amount, energy = (newState.energy - energyCost).coerceAtLeast(0))
                }
            ...
            if (newState.balance < 1000) newState = newState.copy(creditScore = (newState.creditScore - 5).coerceAtLeast(0))
            ...
            if (newState.level > currentState.level) {
                ...
                _history.update { list -> listOf("🎁 LEVEL UP (+${bonus.toInt()}₽)") + list }
            }
    ...
    fun onSalary() {
        if (_state.value.energy < 30) { _errorEvent.value = "Вы валитесь с ног! Поспите."; return }
    ...
    fun onSleep() {
        ...
        _state.update { currentState ->
            val hasCoffee = currentState.inventory.contains("coffee")
            val hasCat = currentState.inventory.contains("cat")
            ...
            // Восстановление (Бонус кофе)
            val moodGain = if (hasCoffee) 20 else 10
            var newMood = (currentState.mood + moodGain).coerceAtMost(100)

            // Бонус кота (защита от депрессии)
            if (hasCat && newMood < 30) newMood = 30
            ...
            if (hasCoffee) msg += " Кофе бодрит!"
            if (hasCat) msg += " Кот мурчит."
            if (loanInterest > 0) msg += " Долг вырос (-${loanInterest.toInt()})."
